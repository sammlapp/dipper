...
function ReviewSettings({ onSettingsChange, onReRenderSpectrograms, onClearCache, currentSettings, onSaveConfig, onLoadConfig, csvColumns, onApplyClassesOnly, onApplyClassesAndSubset }) {
  ...
  useEffect(() => {
    ...
    setLocalManualClasses(settings.manual_classes);
  }, [settings.bandpass_range, settings.reference_frequency, settings.image_width, settings.image_height, settings.manual_classes]);
      ...
      {settings.review_mode === 'multiclass' && (
        <div className="settings-section">
          ...
          <div className="input-group">
            ...
            <textarea
              id="manual_classes"
              value={localManualClasses}
              onChange={(e) => setLocalManualClasses(e.target.value)}
              placeholder="Enter class names, one per line:&#10;bird&#10;car&#10;dog&#10;noise"
              rows="4"
              className="class-input"
            />
            <small className="help-text">
              Enter each class name on a new line. Click a button below to apply changes.
            </small>
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
              <button
                onClick={() => {
                  if (onApplyClassesOnly) {
                    onApplyClassesOnly(localManualClasses);
                  }
                }}
                style={{
                  flex: 1,
              ...
              <button
                onClick={() => {
                  if (onApplyClassesAndSubset) {
                    onApplyClassesAndSubset(localManualClasses);
                  }
                }}
                style={{
                  flex: 1,
            ...
            <small className="help-text" style={{ display: 'block', marginTop: '5px' }}>
              <strong>Change Visible Classes:</strong> Only updates the class list for selection (doesn't modify clip labels)<br />
              <strong>Change Classes + Subset Labels:</strong> Updates class list AND removes any labels not in the list
            </small>
          </div>
        </div>
      )}
